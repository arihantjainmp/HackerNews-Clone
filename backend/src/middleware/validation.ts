...
export const createPostSchema = Joi.object({
  ...
  text: Joi.string().max(10000).trim().optional().messages({
...
export const getPostsQuerySchema = Joi.object({
  ...
  sort: Joi.string().valid('new', 'top', 'best').optional().messages({
    'any.only': 'Sort must be one of: new, top, best',
  }),
  q: Joi.string().trim().optional().messages({
...
export const createCommentSchema = Joi.object({
  content: Joi.string().min(1).max(10000).trim().required().messages({
